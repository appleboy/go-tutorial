...
**注意:** 本文件屬於 Google 內部 [Go 風格](/) 系列文件之一。本份文件具有 **[規範性](/#normative) 與 [典範性](/#canonical)**。詳細說明請見[總覽](/#about)。
...
- 同一套件中良好的[可執行範例](https://cs.opensource.google/go/go/+/refs/tags/go1.19.2:src/sort/example_search_test.go),它同時嘉惠使用者 (這些範例[會顯示在 godoc 中](https://pkg.go.dev/sort#pkg-examples)) 與維護者 (這些範例[會作為測試的一部分執行](/decisions/#examples))。
...
- 為了避免[縮排變化造成的混淆](/decisions/#indentation-confusion) (例如函式宣告、條件判斷)
...
- 使用時不應感到[重複](/decisions/#repetition)
- 將上下文納入考量
- 不重複已經明確的概念

更具體的命名指引請參考[決策](/decisions/#naming)。
