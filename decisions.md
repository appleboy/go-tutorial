...
**注意:** 本文件屬於 Google 內部 [Go 風格](index.md) 系列文件之一。本份文件具 **[規範性](index.md#normative),但不具 [典範性](index.md#canonical)**,屬於[核心風格指南](guide.md)的下級文件。詳細說明請見[總覽](index.md#about)。
...
本文件**並非詳盡無遺**,且會隨時間擴充。當[核心風格指南](guide.md)與此處內容衝突時,**以風格指南為準**,本文件應據此更新。

完整的 Go 風格文件清單請參考[總覽](https://google.github.io/styleguide/go#about)。

下列章節已從風格決策搬移至指南其他位置:

- **MixedCaps**:見 [guide#mixed-caps](guide.md#mixed-caps)
  <a id="mixed-caps"></a>

- **格式化 (Formatting)**:見 [guide#formatting](guide.md#formatting)
  <a id="formatting"></a>

- **行長度 (Line Length)**:見 [guide#line-length](guide.md#line-length)
  <a id="line-length"></a>
...
關於命名的整體性指引,請參考[核心風格指南](guide.md#naming)中的命名章節。下列各節對命名的特定面向作進一步澄清。
...
- [關於所謂「工具套件」的指引](best-practices.md#util-packages)
...
長註解行應折行,以確保在不會自動折行的工具中也能閱讀原始碼。如果不確定要折在哪,80 或 100 欄是常見選擇。但這不是硬性截斷;在某些情況下,把長文字折開反而有害。對折行的特定欄寬不作要求。請力求在同一份檔案內保持[一致](guide.md#consistency)。
...
[Naked return][Naked returns] 只在小函式中可被接受。一旦函式變成中等大小,就明確寫出回傳值。同樣地,不要只是為了能用 naked return 而為回傳參數命名。在你的函式中,[清晰性](guide.md#clarity)永遠比省幾行更重要。
...
匯入套件的本地名稱必須遵循[套件命名指引](#package-names),包括禁用底線與大寫字母。請[一致地](guide.md#consistency)為相同的匯入套件採用相同的本地名稱。

當與其他匯入發生名稱衝突時,匯入套件*必須*改名。(由此延伸出:[好的套件名稱](#package-names)應該不需要改名。) 發生名稱衝突時,優先改名最區域、最專案專屬的匯入。

由 protocol buffer 生成的套件*必須*改名以移除底線,且本地名稱必須以 `pb` 結尾。詳情請參考[proto 與 stub 最佳實踐](best-practices.md#import-protos)。
...
關於錯誤處理的更多討論與範例,請見 [Effective Go](http://golang.org/doc/effective_go.html#errors) 與[最佳實踐](best-practices.md#error-handling)。
...
對於原本會有許多引數的函式呼叫,參考[最佳實踐](best-practices.md#funcargs)中關於縮短呼叫端的選項。
...
可能時,避免對特定函式引數加入行內註解。改用[選項 struct](best-practices.md#option-structure),或在函式文件中加入更多細節。
...
也請參考[何時 panic 是可接受的](best-practices.md#when-to-panic)。
...
在這兩種情況下,這個模式的價值在於 helper 可以在「值」的語境中被呼叫。這些 helper 不應在難以保證錯誤會被攔下的地方,或在錯誤應被[檢查](#handle-errors)的語境 (例如多數 request handler) 中被呼叫。對常數輸入,這允許測試輕易地確保 `Must` 的引數格式良好;對非常數輸入,這允許測試驗證錯誤被[正確處理或傳遞](best-practices.md#error-handling)。

當 `Must` 函式用於測試時,通常應[標記為測試 helper](#mark-test-helpers),並在錯誤時呼叫 `t.Fatal` (使用上的更多考量請參考[測試 helper 中的錯誤處理](best-practices.md#test-helper-error-handling))。

當[一般錯誤處理](best-practices.md#error-handling)是可行的 (包含經過一些重構),就不應使用它們:
...
在[真有需要](guide.md#simplicity)之前,不要建立 interface。聚焦於所需行為,而不是「服務」「儲存庫」這類抽象命名模式。

- 不要為了抽象化或測試,把 RPC 用戶端包成手寫的 interface。改為[使用真實的傳輸層](best-practices.md#use-real-transports) ([測試 RPC][testing RPC])。
...
關於 interface 的更深入討論,請見[最佳實踐中的 interface 章節](best-practices.md#interfaces)。
...
當泛型 (正式名稱「[Type Parameters]」) 能滿足你的業務需求時,就允許使用。在許多應用中,使用既有語言特性 (slice、map、interface 等) 的傳統做法效果同樣好,且不會帶來額外複雜度,因此要小心過早使用。請見[最小機制原則](guide.md#least-mechanism)的討論。
...
- 關於 [logging 錯誤](best-practices.md#error-logging) 與[自訂詳細度](best-practices.md#vlog)的最佳實踐
- 何時及如何用 log 套件[停止程式](best-practices.md#checks-and-panics)
...
Assertion 函式庫嘗試把驗證與失敗訊息產生結合在測試中 (儘管同樣的陷阱也可能適用於其他測試 helper)。關於測試 helper 與 assertion 函式庫的更多區別,請見[最佳實踐](best-practices.md#test-functions)。
...
- [印出 diff](#print-diffs)
- 關於測試 helper 與 assertion helper 的更多區別,請見[最佳實踐](best-practices.md#test-functions)
...
**最佳實踐:** 關於何時應用 `t.Fatal` 的更多討論,請見[最佳實踐](best-practices.md#t-fatal)。
...
Go 中的錯誤值通常含有給人看的部分,以及給語意控制流程用的部分。測試應只測試可被穩定觀察的語意資訊,而非為人類除錯而設的顯示資訊,因為後者經常未來會變動。關於以語意意義建構錯誤的指引,請見[最佳實踐關於錯誤的章節](best-practices.md#error-handling)。如果你不能控制的相依回傳的錯誤語意資訊不足,考慮對該套件擁有者開立 bug 改善 API,而不是依賴解析錯誤訊息。
...
> **注意:** 如果測試使用 [`cmpopts.EquateErrors`],但其所有 `wantErr` 值都只是 `nil` 或 `cmpopts.AnyError`,那麼使用 `cmp` 就是[不必要的機制](guide.md#least-mechanism)。把 want 欄位改為 `bool` 來簡化程式碼,然後就可以用簡單的 `!=` 比較。
>
> ```go
> // Good:
> err := f(test.input)
...
當每筆表格項目根據輸入有特化邏輯時,測試程式碼中的更複雜邏輯 (例如基於測試 setup 條件式差異的複雜錯誤檢查 (常常基於 table 測試輸入參數)) 可能[難以理解](guide.md#maintainability)。如果 test case 邏輯不同但 setup 相同,在單一測試函式內使用一連串[子測試](#subtests)可能更易讀。測試 helper 也可協助簡化測試 setup 以維持測試本體的可讀性。
...
**提示:** 關於測試 helper 與 assertion helper 的更多區別,請見[最佳實踐](best-practices.md#test-functions)。
